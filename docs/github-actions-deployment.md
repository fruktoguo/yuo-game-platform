...
生产服务器继续使用人工审核并固定安装的 Compose 与 Caddy 配置。自动流程只能更新 `GAME_PLATFORM_IMAGE` 并重建已有服务，不能通过仓库中的 Compose 增加宿主机挂载、特权容器或额外端口。需要修改服务拓扑、内存配额、数据卷或反向代理时，必须单独验证并人工安装配置。
...
部署前会备份 `.env`、Compose、PostgreSQL、生命世界和贪吃蛇战绩到 `backups/github-*`。失败时自动恢复上一镜像配置，但不会自动覆盖数据库或游戏存档，避免回滚过程丢失部署期间的新写入。每次保留最近五个 `sha-*` 镜像标签用于快速回滚。

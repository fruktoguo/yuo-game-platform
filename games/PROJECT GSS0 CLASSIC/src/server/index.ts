...
const port = parsePort(process.env.PORT, 3105);
const host = process.env.HOST ?? '0.0.0.0';
const dataPath = process.env.DATA_PATH ?? resolve(process.cwd(), 'data/snake-classic-profiles.json.gz');
const app = express();
...
const authBridge = new GameAuthBridge({
  gameId: 'neon-snake-arena-classic',
  platformInternalUrl: process.env.PLATFORM_INTERNAL_URL ?? 'http://127.0.0.1:3100',
  serviceToken: process.env.PLATFORM_SERVICE_TOKEN ?? 'dev-snake-classic-service-token-change-before-production-2026',
  sessionSecret: process.env.GAME_SESSION_SECRET ?? 'dev-snake-classic-game-session-secret-change-before-production-2026',
  cookieName: 'yuo_snake_classic_game_session',
  secureCookies: process.env.COOKIE_SECURE === 'true',
});
io.use(authBridge.socketMiddleware());
const hub = await RoomHub.create(io, dataPath);
const presenceReporting = startPresenceReporting(authBridge.serviceClient, () => hub.listOnlinePlayers());

app.disable('x-powered-by');
app.use(authBridge.createRouter());
app.get('/health', (_request, response) => {
  response.json({ ok: true, service: 'neon-snake-arena-classic', ...hub.getHealth() });
});
...
httpServer.listen(port, host, () => {
  console.info(`Neon Snake Classic P2P 大厅已启动：http://${host}:${port}`);
});

...
describe('返回大厅导航', () => {
  it('优先返回实际来源大厅，不受过期配置地址影响', () => {
    expect(navigation.resolveLobbyUrl({
      currentHref: 'https://game.dstopology.com/snake-classic/?launch_code=ticket&lobby_url=https%3A%2F%2Fold.example.com%2F',
      referrer: 'https://game.dstopology.com/',
    })).toBe('https://game.dstopology.com/');
  });

  it.each([
    'https://game.dstopology.com/snake-classic',
    'https://game.dstopology.com/snake-classic/',
    'https://game.dstopology.com/snake-classic/index.html',
    'https://game.dstopology.com/snake-classic/?launch_code=again&lobby_url=https%3A%2F%2Fgame.dstopology.com%2Fsnake-classic%2F',
  ])('拒绝指回游戏或递归启动的大厅地址：%s', (lobbyUrl) => {
    const current = new URL('https://game.dstopology.com/snake-classic/');
  ...
  it('本地开发和直接打开文件时都回到本地大厅', () => {
    expect(navigation.resolveLobbyUrl({ currentHref: 'http://127.0.0.1:5179/index.html' }))
      .toBe('http://127.0.0.1:3100/');
    expect(navigation.resolveLobbyUrl({ currentHref: 'file:///D:/games/PROJECT%20GSS0%20CLASSIC/index.html' }))
      .toBe('http://127.0.0.1:3100/');
  });
});

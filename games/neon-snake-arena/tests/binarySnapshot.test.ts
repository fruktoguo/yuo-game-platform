...
import { SnapshotBuffer } from '../src/client/game/SnapshotBuffer';
...
describe('Ultra 快照补间', () => {
  it('在网络快照之间平滑补间蛇头、蛇身和敌蛇', () => {
    const buffer = new SnapshotBuffer();
    buffer.push(snapshotAt(1, 0), 0);
    buffer.push(snapshotAt(2, 10), 100);

    const sampled = buffer.sample(160)!;
    expect(sampled.players[0].col).toBeCloseTo(5, 5);
    expect(sampled.players[0].segments[0].col).toBeCloseTo(4, 5);
    expect(sampled.enemies[0].col).toBeCloseTo(7, 5);
  });

  it('拒绝重复或倒序 tick，保留最新权威帧', () => {
    const buffer = new SnapshotBuffer();
    expect(buffer.push(snapshotAt(4, 4), 0)).toBe(true);
    expect(buffer.push(snapshotAt(4, 8), 10)).toBe(false);
    expect(buffer.push(snapshotAt(3, 9), 20)).toBe(false);
    expect(buffer.getLatest()?.players[0].col).toBe(4);
  });

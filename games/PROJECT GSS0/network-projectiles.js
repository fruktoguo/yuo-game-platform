(function installProjectileRuntime(root) {
  ...
  class ProjectileRuntime {
    ...
    update(delta, targetById, arena) {
      ...
      const maximum = this.gridSize - 0.5;
      for (const projectile of this.items) {
        ...
        const hitHorizontal = projectile.col < PROJECTILE_MIN || projectile.col > maximum;
        const hitVertical = projectile.row < PROJECTILE_MIN || projectile.row > maximum;
        if ((hitHorizontal || hitVertical) && projectile.bounces > 0) {
          projectile.col = Math.max(PROJECTILE_MIN, Math.min(maximum, projectile.col));
          projectile.row = Math.max(PROJECTILE_MIN, Math.min(maximum, projectile.row));
        ...
        projectile.x = arena.left + (projectile.col + 0.5) * arena.cellSize;
        projectile.y = arena.top + (projectile.row + 0.5) * arena.cellSize;

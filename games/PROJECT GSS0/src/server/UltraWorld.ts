...
export class UltraWorld {
  ...
  setAutopilot(accountId: string, enabled: boolean, autoSelectModules: boolean, autoRestart: boolean): boolean {
    ...
    if (enabled) player.dashHeld = false;
  ...
  applyInput(accountId: string, payload: PlayerMovementState, now = this.now): boolean {
    ...
    player.dashHeld = !player.ghost && payload.dashHeld;
    PLAYER_DASH.advance(player, player.dashHeld, 0, PLAYER_DASH_TUNING);
  ...
  applyCollisionClaim(accountId: string, claim: PlayerCollisionClaim, now = Date.now()): boolean {
    ...
    if (claim.kind === 'wall') {
      ...
      this.damagePlayerFromCollision(player, PLAYER_WALL_COLLISION_DAMAGE, now, '撞上墙壁');
      return true;
    }
    if (claim.kind === 'self-body') {
      if (!findSelfCollision(player, PLAYER_SELF_COLLISION_RANGE)) return false;
      this.triggerCollisionEcho(player);
      if (!this.hasActiveKnockback(player)) this.damagePlayerFromCollision(player, PLAYER_WALL_COLLISION_DAMAGE, now, '撞上自己的身体');
      return true;
    }
    ...
    this.resolveBodyIntercept(player, enemy, collisionPoint);
    return true;
  }
  ...
  private applyPlayerHeadCollisionClaim(
    ...
    this.recentPlayerHeadCollisionEvents.set(eventId, now);
    if (target.autopilot) {
      this.bounceEntity(target, -event.normalCol, -event.normalRow, PLAYER_COLORS[target.colorIndex]);
    }
  ...
  private applyPlayerBodyCollisionClaim(
    ...
    this.damagePlayerFromCollision(source, PLAYER_OTHER_BODY_COLLISION_DAMAGE, now, '撞上其他玩家的身体');
    return true;
  }
  ...
  private resetPlayerRun(player: PlayerEntity, spawn: GridPoint, now: number): void {
    ...
    player.energy = PLAYER_ENERGY_MAXIMUM;
    player.dashing = false;
    player.dashHeld = false;
    player.dashElapsed = 0;
    player.dashMovementMultiplier = 1;
  ...
  private updateArenaSize(delta: number, presentPlayers: readonly PlayerEntity[]): void {
  ...
  private movePlayer(player: PlayerEntity, delta: number): void {
    ...
    player.speed = this.playerBaseSpeed(player) * slowMultiplier * player.dashMovementMultiplier;
  ...
  private updatePlayerTimers(player: PlayerEntity, delta: number): void {
    ...
    const dashHeld = player.autopilot
      ? false
      : player.dashHeld && this.now - player.lastManualStateAt <= NETWORK_MANUAL_PREDICTION_MS;
    player.dashMovementMultiplier = PLAYER_DASH.advance(player, dashHeld, delta, PLAYER_DASH_TUNING);
  ...
  private updateSpawns(delta: number, players: readonly PlayerEntity[], activePlayers: readonly PlayerEntity[]): void {
  ...
  private resolveBodyIntercept(player: PlayerEntity, enemy: EnemyEntity, collisionPoint: GridPoint): void {
    ...
    if (!enemy.dead) this.bounceEntity(enemy, enemy.col - collisionPoint.col, enemy.row - collisionPoint.row, enemy.color);
  }
  ...
  private updateModules(player: PlayerEntity, delta: number): void {
    for (const segment of player.segments) {
      ...
      const target = segment.module === 'incendiary'
        ? this.highestHealthTarget(segment)
        : this.nearestTarget(player, segment, Number.POSITIVE_INFINITY);
      ...
      switch (segment.module) {
        ...
        case 'flare':
          ...
          break;
        case 'incendiary':
          if (this.spawnShot(player, segment, target, {
            color: MODULE_BY_ID.incendiary.color,
            speed: DESIGNER_BALANCE.moduleIncendiaryProjectileSpeed,
            size: DESIGNER_BALANCE.moduleIncendiaryProjectileSize,
            homing: DESIGNER_BALANCE.moduleIncendiaryHoming,
            burnOnHit: true,
          })) this.playSkillSound(player, 'incendiary');
          segment.timer = this.activeModuleCooldown(player, 'incendiary', segment.moduleLevel);
          break;
        case 'scatter':
          if (target) {
  ...
  private resolveEnemyCollisions(): void {
    ...
    for (let firstIndex = 0; firstIndex < this.enemies.length; firstIndex += 1) {
      ...
      for (let secondIndex = firstIndex + 1; secondIndex < this.enemies.length; secondIndex += 1) {
        ...
        if (!first.dead) this.bounceEntity(first, normal.col, normal.row, first.color, collisionKnockback);
        if (!second.dead) this.bounceEntity(second, -normal.col, -normal.row, second.color, collisionKnockback);
        break;
      }
    }
    ...
    for (const enemy of this.enemies) {
      ...
      if (!enemy.dead) this.bounceEntity(enemy, normalCol, normalRow, enemy.color, collisionKnockback);
    }
  }
  ...
  private checkCollisions(now: number, players: PlayerEntity[], presentPlayers: PlayerEntity[]): void {
    ...
    for (const player of players) {
      ...
      if (player.collisionCooldown <= 0) {
        ...
        if (ownBody) {
          ...
          this.triggerCollisionEcho(player);
          if (!knockedBack) this.damagePlayerFromCollision(player, PLAYER_WALL_COLLISION_DAMAGE, now, '撞上自己的身体');
          if (player.alive) this.bounceEntity(player, player.col - ownBody.col, player.row - ownBody.row, '#f4ffdc');
          continue;
        }
      }
    ...
    for (let leftIndex = 0; leftIndex < players.length; leftIndex += 1) {
      ...
      for (let rightIndex = leftIndex + 1; rightIndex < players.length; rightIndex += 1) {
        ...
        this.bounceEntity(left, normal.col, normal.row, PLAYER_COLORS[left.colorIndex]);
        if (right.alive) this.bounceEntity(right, -normal.col, -normal.row, PLAYER_COLORS[right.colorIndex]);
    ...
    for (const attacker of players) {
      ...
      for (const defender of presentPlayers) {
        ...
        this.bounceEntity(attacker, normal.col, normal.row, PLAYER_COLORS[attacker.colorIndex]);
  ...
  private checkPlayerBodyHit(attacker: PlayerEntity, defender: PlayerEntity, now: number): void {
    ...
    this.damagePlayerFromCollision(attacker, PLAYER_OTHER_BODY_COLLISION_DAMAGE, now, '撞上其他玩家的身体');
  ...
  private damagePlayerFromCollision(player: PlayerEntity, amount: number, now: number, reason: string): boolean {
    if (player.dashing) return false;
    return this.damagePlayer(player, amount, now, reason);
  }
  ...
  private triggerBodyIntercept(player: PlayerEntity, point: GridPoint, enemyAt: GridPoint): void {
  ...
  private highestHealthTarget(origin: GridPoint): EnemyTargetSelection | null {
    let selected: EnemyTargetSelection | null = null;
    let highestHealth = -1;
    for (const target of this.enemies) {
      if (target.dead) continue;
  ...
  private eliminatePlayer(victim: PlayerEntity, killer: PlayerEntity | null, now: number, reason: string): void {
    ...
    victim.dashHeld = false;
    victim.dashElapsed = 0;
    victim.dashMovementMultiplier = 1;
    victim.paused = false;
  ...
  private bounceEntity(entity: PlayerEntity | EnemyEntity, normalX: number, normalY: number, color: string, extraImpulseMultiplier = 1, mitigatePlayerCollision = false): void {
...
function toPlayerView(player: PlayerEntity): UltraPlayerView {
  return {
    ...
    dashElapsed: player.dashElapsed,
    slow: player.slow,

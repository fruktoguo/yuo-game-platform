...
export class UltraWorld {
  ...
  step(deltaSeconds: number, now = Date.now()): void {
    ...
    for (const projectile of this.projectiles) {
      if (projectile.life <= 0 || !isProjectileInside(projectile)) {
  ...
  private updateEnemies(delta: number, activePlayers: PlayerEntity[], presentPlayers: PlayerEntity[]): void {
    ...
    for (const enemy of this.enemies) {
      ...
      if (enemy.collisionCooldown <= 0) {
        ...
        const avoidance = this.playerBodyAvoidance(enemy, activePlayers);
  ...
  private resolveEnemyCollisions(): void {
    for (let firstIndex = 0; firstIndex < this.enemies.length; firstIndex += 1) {
      ...
      for (let secondIndex = firstIndex + 1; secondIndex < this.enemies.length; secondIndex += 1) {
        ...
        if (Math.hypot(first.col - second.col, first.row - second.row) >= this.playerHeadRadiusCells() * 2) continue;
  ...
  private playerBodyAvoidance(enemy: EnemyEntity, activePlayers: PlayerEntity[]): { angle: number; strength: number } | null {
    let combinedX = 0;
    let combinedY = 0;
  ...
  private maximumModuleCount(id: ModuleId, players = this.activePlayers()): number {

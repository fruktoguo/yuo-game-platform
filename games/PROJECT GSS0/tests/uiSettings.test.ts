...
describe('界面设置', () => {
  it('使用正式游戏名并在左上品牌卡显示当前版本', () => {
    ...
    expect(indexHtml).toContain('<span class="brand-version" aria-label="游戏版本 V149">V149</span>');
  ...
  it('新存档默认跟随蛇头并可切换固定视角与滚轮缩放', () => {
    ...
    expect(indexHtml).toContain('src="camera-projection.js?v=149"');
  ...
  it('升级卡展示机体等级变化且机体架显示槽位占用', () => {
    expect(indexHtml).toContain('src="module-progression.js?v=149"');
  ...
  it('九个右上角按钮使用零延迟自定义提示', () => {
    ...
    expect(indexHtml).toContain('src="module-catalog.js?v=149"');

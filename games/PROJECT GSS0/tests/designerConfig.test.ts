...
describe('设计配置', () => {
  ...
  it('本地编辑器与运行时配置使用完全相同的参数和机体 ID', () => {
    ...
    expect(moduleProgressionSource).toContain('config?.schemaVersion !== 57');
    expect(new Set(parameterKeys).size).toBe(327);
  ...
  it('全部机体共用唯一描述目录且被动参数明确', () => {
    ...
    expect(editorHtml).toContain('src="module-catalog.js?v=155"');
    expect(editorHtml).toContain('src="module-progression.js?v=155"');

...
function Test-DesignerConfig {
  ...
  try {
    ...
    $currentContent = [IO.File]::ReadAllText($configPath)
    ...
    return $config.schemaVersion -eq $currentConfig.schemaVersion -and $null -ne $config.balance -and $null -ne $config.waveEnemyCountSchedule -and $null -ne $config.moduleCooldownPercentages -and $null -ne $config.moduleNames -and $null -ne $config.moduleInitialUpgrades -and $null -ne $config.moduleStates
  } catch {
    return $false
  }
}

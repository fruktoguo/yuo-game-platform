...
- The current game version is `V87`.
...
- The canonical module categories are `进攻`, `生存`, `辅助`, and `发育`. In user conversation, treat the legacy names `输出` as `进攻` and `防御` as `生存` without asking for clarification.
